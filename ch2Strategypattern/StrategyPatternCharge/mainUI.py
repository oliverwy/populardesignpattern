...
from ch2Strategypattern.StrategyPatternCharge.CashSuper import *
class Ui_Form(object):
    ...
    def on_click(self):  # 未做错误保护，写实际应用的时候注意业务逻辑异常保护
        ...
        if type=="正常收费":
            cs=CashContext(CashNormal())
        elif type=="满300返100":
            cs=CashContext(CashReturn(300,100))
        elif type=="打8折":
            cs=CashContext(CashRebate(0.8))
        elif type=="打7折":
            cs=CashContext(CashRebate(0.7))
        elif type=="打5折":
            cs=CashContext(CashRebate(0.5))
        # cs=CashFactory.createCashAccept(self.cbxType.currentText())
        totalPrice=0
        totalPrice=cs.getResult(eval(self.txtPrice.text())*eval(self.txtNum.text()))
